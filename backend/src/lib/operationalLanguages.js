...
export function getOperationalLanguageByKey(key) {
  const normalized = normalizeLanguageKey(key)
  return getOperationalLanguages().find((language) => language.key === normalized) ?? null
}

export function getOperationalLanguageByLabel(label) {
...
export async function ensureCoreOperationalLanguages(pool, userId) {
  const languages = getOperationalLanguages().filter((language) => language.isCore)
    ...
    await pool.query(
      `
        ...
        DO UPDATE SET
          ...
          updated_at = now()
      `,
      [userId, language.key, language.label, language.targetLanguage || null]
    )
  }
}

export async function readUserOperationalLanguages(pool, userId) {
  await ensureCoreOperationalLanguages(pool, userId)
  const { rows } = await pool.query(
    `
      SELECT language_key, label, target_language, is_core, active, updated_at
      FROM user_operational_languages
      WHERE user_id = $1::uuid
      ORDER BY is_core DESC, label ASC
    `,
    [userId]
  )
  const activeByKey = new Map(rows.map((row) => [row.language_key, row]))
  return getOperationalLanguages().map((language) => {
    const row = activeByKey.get(language.key)
    return {
      ...language,
